...
def naive_collect_forward_input(
    ...
    collected_x = np.empty((x.shape[0], x.shape[1], x.shape[2] * mp_size),dtype=x.dtype)
    temp_buffer = np.empty((mp_size, batch_size, seq_length, part_in_dim), dtype=x.dtype)
    mp_comm.Allgather(x_contiguous,temp_buffer)
    for i in range(mp_size):
        collected_x[:, :, i*part_in_dim:(i+1)*part_in_dim] = temp_buffer[i]
    return collected_x
...
def naive_collect_forward_output(
    ...
    collected_out = np.empty((out.shape[0], out.shape[1], out.shape[2] * mp_size),dtype=out.dtype)
    temp_buffer = np.empty((mp_size, batch_size, seq_length, part_out_dim), dtype=out.dtype)
    mp_comm.Allgather(out_contiguous, temp_buffer)
    collected_out = np.empty((batch_size, seq_length, mp_size * part_out_dim), dtype=out.dtype)
    for i in range(mp_size):
        collected_out[:, :, i*part_out_dim:(i+1)*part_out_dim] = temp_buffer[i]
...
def naive_collect_backward_x(
    ...
    # part_in_dim = in_dim // mp_size
    # grad_x_contiguous = np.ascontiguousarray(grad_x)
    # buffer= np.empty((mp_size, batch_size, seq_length, in_dim), dtype=grad_x.dtype)
    # mp_comm.Allgather(grad_x_contiguous, buffer)
    # summed = np.sum(buffer, axis=0)
    # rank = mp_comm.Get_rank()
    # collected_grad_x = summed[:, :, rank*part_in_dim:(rank+1)*part_in_dim]
    # return collected_grad_x
    batch_size, seq_length, in_dim = grad_x.shape
    part_in_dim = in_dim // mp_size
    grad_x = grad_x.reshape(batch_size * seq_length, mp_size, part_in_dim)
    send_buffer = grad_x.transpose(1, 0, 2).reshape(-1)
    recv_buffer = np.empty(batch_size * seq_length * part_in_dim, dtype=grad_x.dtype)
    mp_comm.Reduce_scatter(send_buffer, recv_buffer, op=MPI.SUM)
    collected_grad_x = recv_buffer.reshape(batch_size, seq_length, part_in_dim)
    return collected_grad_x
